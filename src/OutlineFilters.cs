...
internal static class OutlineFilters
{
    ...
    // fire-specific, so a lamp or a light switch is not caught. "InteractiveFire" is the open campfire
    // (meshes "LargeFire"/"Embers") whose interactable node and ObjectRoot are unnamed, so it is matched
    // at the render root the outline climbs to.
    private static readonly string[] FireMarkers = { "firebarrel", "fire-interactable", "Fire-Camp", "InteractiveFire" };
    ...
    // An object that carries an explosive trigger in the data but is not a canister hazard to mark red,
    // so the mod never marks it. A fire extinguisher ("Deco-Industrial-FireExtinguisher", node
    // "fire-extinguisher-physics") only rockets off on the applied force and deals no damage (verified
    // in-game). A barricade section ("barricade-section-short") is destructible cover, not a canister to
    // avoid, so it is excluded by request. Matched by fragment, so the deco root and its child nodes are
    // both caught.
    private static readonly string[] ExcludedExplosiveMarkers = { "extinguisher", "barricade" };
    ...
    // True when the name marks an object with an explosive trigger that the mod does not mark red: a
    // fire extinguisher (harmless in normal play) or a barricade section (destructible cover).
    public static bool IsExcludedExplosive(string name) => ContainsAny(name, ExcludedExplosiveMarkers);
